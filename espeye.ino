...
#define DISPLAY_DC      D4 // Data/command pin for BOTH displays
#define DISPLAY_RESET   D6 // Reset pin for BOTH displays
...
struct {
  ...
  TFT_eSPI(),SELECT_L_PIN,{0,NOBLINK},
...
void setup(void) {
  Serial.begin(115200);
  eye[0].tft.init();
  eye[0].tft.fillScreen(TFT_BLACK);
  eye[0].tft.setRotation(0);
...
void drawEye( // Renders one eye.  Inputs must be pre-clipped & valid.
  ...
  eye[e].tft.setAddrWindow(0, 0, 0, 0);
 ...
 // Autonomous X/Y eye motion
  ...
  if(eyeInMotion) {                       // Currently moving?
    if(dt >= eyeMoveDuration) {           // Time up?  Destination reached.
      ...
      eyeMoveDuration  = random(2000000L); // 0-3 sec stop
...
void startTelegramBot(){
  ...
  if (myBot.testConnection())
      myBot.sendMessage(BOT_ID, "\xF0\x9F\x91\x80 Algo foi visto pelo beholder.");   
}
